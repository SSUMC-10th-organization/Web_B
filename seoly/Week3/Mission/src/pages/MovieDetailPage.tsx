	...
	useEffect(() => {
		const fetchMovieDetail = async () => {
			...
			try {
				...
				const directorInfo = creditData.crew.find((person) => person.job === "Director");
					...
					<div className="bg-gray-800/50 p-6 rounded-xl border border-gray-600">
						...
						<div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-5 gap-x-4 gap-y-10">
							{director && (
          					...
          					<img
            					src="https://via.placeholder.com/150?text=Director" alt={director.name}
            					className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110"/>
									</div>
									<p className="text-sm font-bold truncate w-full">{director.name}</p>

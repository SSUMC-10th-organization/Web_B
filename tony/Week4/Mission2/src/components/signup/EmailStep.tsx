...
const validate = (values: EmailFields): Partial<Record<keyof EmailFields, string>> => {
...
function EmailStep({ onNext }: Props) {
  const { values, errors, touched, handleChange, handleBlur, isValid } = useForm<EmailFields>(
    { email: "" },
    validate
  );
